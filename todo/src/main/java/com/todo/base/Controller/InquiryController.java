...
public class InquiryController {
	...
	@Autowired
	private InquiryContentInsertService inquiryContentInsertService;
	
	@GetMapping("inquiry/form")
	public String form(InquiryForm inquiryForm, Model model) {
		model.addAttribute("title", "お問合わせ");
		return "inquiry/form";
	}
	...
	public String confirm(@Validated InquiryForm inquiryForm, BindingResult result, Model model) {
		if(result.hasErrors()) {
			model.addAttribute("title", "お問合わせ内容エラー");
			return "inquiry/form";
		}
		model.addAttribute("title", "お問合わせ内容確認");
	...
	public String complete(@Validated InquiryForm inquiryForm, BindingResult result, Model model,RedirectAttributes redirectAttributes) {
		...
		inquiryContentInsertService.inquiryContentInsert(inquiryEntity);
